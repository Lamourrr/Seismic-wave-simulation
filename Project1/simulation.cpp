...
int PhaseShift()
{
	...
	for (Iw = 0; Iw < Nw / 2 + 1; Iw++)
	{
		...
		for (Iz = Nz - 1; Iz > 0; Iz--)
		{
			...
			if (FrmNewWfld(fp_Wfld0r, fp_Wfld0i, Wfldr, Wfldi, Absb, Iz, Iw) != 1) {
				printf("FrmNewWfldError"); exit(0);
			}
			// 4.2.3 Wave field Extrapolating One Depth Step Error");exit(0);}
			if (MoveOneDz(Wfldr, Wfldi, (float)(Vlcty[Iz] / 2.), Dkx, Dw, Iw) != 1) {
				printf("WfldMoveOneStepError"); exit(0);
			}
		}
...
int FrmNewWfld(FILE* fp_Wfld0r, FILE* fp_Wfld0i, float Wfldr[], float Wfldi[], float Absb[], int Iz, int Iw)
{
	...
	if (ReadIxIzIwToIwIzIx(fp_Wfld0r, fp_Wfld0i, Wfld0r, Wfld0i, Iz, Iw) != 1) {
		printf("exp_ikzDzis error"); exit(0);
	}
	// Form New Wave Field
	for (Ix = 0; Ix < Nx; Ix++)
	{
...
// 06.1.3.2: kkfft(Wfldr, Wfldi,Nx,j)
	...
	if (kkfft(Wfldr, Wfldi, Nkx, 1) != 1)//37. FFT or IFFT
	{
		printf("FFT is error");
		exit(0);
	}
...
int exp_ikzDz(float eikzdz[], int Ix, float Vc, int Iw, float Dw, float Dkx)
{
	...
	eikzdz[0] = 0.;
	eikzdz[1] = 0.;
	kz = sqrt(pow(Iw * Dw / Vc, 2) - pow(Ix * Dkx, 2));//38. Kz computing?
...
int Frqcy2Time()
{
	...
	long AddFrmStrt;
	if ((fp_Wfldr = fopen("Wfldr.dat", "rb")) == NULL) {
		printf("Connot openWfldr.dat"); exit(0);
	}
	if ((fp_Wfldi = fopen("Wfldi.dat", "rb")) == NULL) {
		printf("Connot openWfldi.dat"); exit(0);
	}
	if ((fp_Record = fopen("Record.dat", "wb")) == NULL) {
		printf("Connot openRecord.dat"); exit(0);
	}
	for (Ix = 0; Ix < Nx; Ix++)
	{
